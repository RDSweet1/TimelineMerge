...
1. **Pre-loads common documents ONCE** (config, epics, PRD, tech spec, workflow-status, architecture)
...
4. **Passes document context** to each sub-agent by including full content in the Task tool prompt parameter
...
- Include pre-loaded document content directly in the Task tool's prompt parameter
...
**Document Passing Mechanism:**
The SM orchestrator (the agent executing this workflow) reads all required documents ONCE in Step 0, then passes them to sub-agents as follows:
1. SM reads: config.yaml, epics.md, PRD.md, workflow-status.yaml, architecture.md, tech-spec-epic-X.md
2. SM stores document contents in its own working memory for the duration of the workflow
3. When spawning sub-agents via Task tool, SM includes relevant document contents directly in the prompt parameter
4. Sub-agents receive documents as context and do NOT need to read files from disk
5. This eliminates ~80% of duplicate file reads across the 6+ agent spawns in this workflow

**Efficiency Gains:**
- **Before (without SM orchestration):** ~34+ file reads with duplicates across agents
- **After (with SM orchestration):** ~6 file reads total (80% reduction)
- **Token savings:** ~50k+ tokens across 6+ agent spawns
- **Time savings:** 6-12 seconds eliminated from file search and duplicate reads. Also, this eliminates all the time wasted when agents are waiting for user response to continue work

## What This Workflow Does

### **Phase 0: Cleanup (Conditional)**
1. **Dev Agent** approves previous story (if exists) - marks Done, advances queue (`story-done` workflow)
...
### **Step 0: SM Initialization (CRITICAL)**
...
**BEFORE executing any sub-agents, the SM agent MUST:**
...
1. **Load all documents from `context_documents` section:**
   ...
   - workflow_status: "{output_folder}/bmm-workflow-status.yaml"
...
3. **Store all documents in SM agent memory:**
   - Documents are now loaded ONCE
   - Will be passed to ALL sub-agents via Task tool prompts
   - No sub-agent needs to read these files again
   - SM agent maintains this context throughout entire workflow

**Token Efficiency:**
- Documents loaded: ~6 files (~15k-20k tokens total)
- Documents passed via prompt: Included in each sub-agent Task call
- Estimated savings: ~50k+ tokens across 6+ agent spawns (SM × 3, Architect × 2, Dev × 1)
- Each sub-agent receives pre-loaded context in their Task prompt
...
**SM agent reads workflow-status from memory (already loaded in Step 0):**
```bash
# No file read needed - workflow_status already loaded
# Extract:
...
**CRITICAL:** SM orchestrator MUST use the Task tool to spawn a sub-SM agent. Do not just describe what should happen - **actually execute the Task tool call**.
...
```
Use Task tool with:
- subagent_type: "sm" (or appropriate agent type for create-story)
- description: "Create Story {epic_num}.{story_num}"
- prompt: Include pre-loaded context (epics, PRD, tech_spec, workflow-status, config, architecture) in the prompt text
```

**Inputs:**
- Epic number (from workflow-status already loaded)
- Story number (from workflow-status - advanced by Step 1 if previous story existed)
- Non-interactive mode: `true`
- **Context passed in prompt:** epics, PRD, tech_spec, architecture (from SM orchestrator memory)

**Expected Output:**
- Story file created at `{story_dir}/story-{epic_num}.{story_num}.md`
- Story status: Draft

**Note:** Sub-SM agent receives pre-loaded documents via Task tool prompt parameter - no file reads needed!

### **Step 5: Spawn Architect Agent for Review**

**CRITICAL:** SM orchestrator MUST use the Task tool to spawn the architect-reviewer agent.
...
- **Context passed in prompt:** tech_spec, epics, architecture (from SM orchestrator memory)
...
**Note:** Architect agent receives pre-loaded documents via Task tool prompt parameter - no duplicate file reads!
